	...
	class NovelDocumentResolver(
		...
		fun resolve(uri: Uri): NovelDocument {
			...
			val format = when {
				...
				displayName.lowercase().endsWith(".docx") -> "docx"
				else -> error("无法识别小说文件类型")
			}
			val resolvedMime = when (format) {
				"epub" -> "application/epub+zip"
				"docx" -> "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
				else -> "text/plain"
			}
			return NovelDocument(displayName.substringBeforeLast('.').ifBlank { "未命名小说" }, format, resolvedMime)
		}
	}
