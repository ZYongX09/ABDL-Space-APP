...
public class CustomWelcomeFragment extends InstanceCatalogFragment {
	...
	protected void updateFilteredList(){
		...
		// Update button states based on whether an instance is selected
		updateButtonStates();
	}
	...
	private void updateButtonStates(){
		...
		Button oauthButton = rootView.findViewById(R.id.btn_oauth);
		Button passwordButton = rootView.findViewById(R.id.btn_password);
		
		boolean hasInstance = chosenInstance != null;
		if(oauthButton != null) oauthButton.setEnabled(hasInstance);
		if(passwordButton != null) passwordButton.setEnabled(hasInstance);
	}
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		// Set up OAuth button
		View oauthButton = view.findViewById(R.id.btn_oauth);
		if (oauthButton != null) {
			oauthButton.setOnClickListener(v -> {
				// Trigger the same logic as the next button
				String domain = getCurrentSearchQuery();
				if (domain.isEmpty() && chosenInstance != null) {
					domain = chosenInstance.domain;
				}
				if (!domain.isEmpty()) {
		...
		// Set up password login button
		View passwordButton = view.findViewById(R.id.btn_password);
		if (passwordButton != null) {
			passwordButton.setOnClickListener(v -> onPasswordLoginClick());
		}
	}
	...
	private void onPasswordLoginClick() {
		...
		// Navigate to password login fragment
		Bundle args = new Bundle();
		args.putString("domain", domain);
		Nav.go(getActivity(), org.joinmastodon.android.fragments.auth.LoginFragment.class, args);
	}
